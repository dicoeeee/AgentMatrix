...
function driverTemplate() {
  return [
    ...
    "- `agentmatrix driver record-event <run-id> < event.json` only for compact platform summaries AgentMatrix core cannot know directly.",
    "",
    "AgentMatrix core records deterministic Run Trace boundaries automatically. Record only compact platform summaries such as executor subagent invocation, verifier subagent invocation, checker shard count, notable command summaries, or Stage Log links.",
    "Use the Stage Log paths from Stage Invocation JSON: `executor.log`, `verifier.log`, and the `subagents/` directory for child subagent logs.",
    "Executor, verifier, and child subagents must write stage reports, evidence, and logs; do not ask them to call `record-event` directly.",
    "",
